...
echo "TCP_BBR sudah dipasang."
echo "#############################################"
return 1
fi
echo "Memasang TCP_BBR..."
modprobe tcp_bbr
...
if [ -n "$(sysctl net.ipv4.tcp_available_congestion_control | grep bbr)" ] && [ -n "$(sysctl net.ipv4.tcp_congestion_control | grep bbr)" ] && [ -n "$(lsmod | grep "tcp_bbr")" ];then
	echo "BBR telah menempel."
	sleep 2
else
	echo "BBR tidak bisa ditempel."
	sleep 2
fi
echo "#############################################"
}
...
Optimize_Parameters(){
echo "#############################################"
echo "Sedang Mengoptimalkan Parameter..."
...
rm -f /root/bbrfast.sh
echo '============================================================'
echo '    Pemasangan BBR Selesai. Silahkan restart server anda    '
echo '============================================================'
sleep 5
